...
function SendWhatsAppPanel({
  ...
  async function submit(e: FormEvent) {
    ...
    setSending(true);
    try {
      const res = await api.sendWhatsApp({ clientId, body: body.trim() }, token);
      setSid(res.messageSid);
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : "Something went wrong");
    } finally {
  ...
  if (sid) {
    return (
      <div className="bg-green-50 border border-green-200 rounded-xl p-5 mb-6 flex items-start justify-between gap-4">
        <div>
          <p className="font-semibold text-green-800 mb-1">WhatsApp sent</p>
          <p className="text-sm text-green-700">
            Twilio SID <span className="font-mono">{sid}</span>
          </p>
        </div>
  ...
  return (
    <form onSubmit={submit} className="bg-white border border-gray-200 rounded-xl p-5 mb-6">
      <div className="flex items-center justify-between mb-3">
        <h2 className="font-semibold text-gray-900">Send WhatsApp</h2>
        <button type="button" onClick={onClose} aria-label="Cancel" className="text-gray-400 hover:text-gray-600">
          <X size={16} />
        </button>
      </div>
      <p className="text-xs text-gray-500 mb-3">
        To: <span className="font-mono">{clientPhone || "(no phone on file)"}</span>
        <span className="ml-2 text-gray-400">(recipient must have opted into your WhatsApp sender — sandbox: text &quot;join &lt;keyword&gt;&quot; to your Twilio number first)</span>
      </p>
      <textarea
        className="input w-full font-sans"
        rows={5}
      ...
      />
      <div className="flex items-center justify-between mt-2">
        <span className="text-xs text-gray-400">{body.length}/4096</span>
        <div className="flex gap-2">
          <button type="button" onClick={onClose} disabled={sending} className="text-sm text-gray-500 hover:text-gray-700 px-3 py-1.5">
            Cancel
          </button>
          <button
            type="submit"
            disabled={sending || !body.trim() || !clientPhone}
            className="bg-emerald-600 text-white px-4 py-1.5 rounded-lg text-sm font-medium hover:bg-emerald-700 disabled:opacity-40"
          >
            {sending ? "Sending…" : "Send"}
          </button>
        </div>
    ...
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => setWhatsapping((v) => !v)}
            className="text-sm border border-emerald-300 text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-lg hover:bg-emerald-100 flex items-center gap-1.5"
          >
